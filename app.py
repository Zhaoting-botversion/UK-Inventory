  ...
  (() => {{
    ...
    document.addEventListener("click", (event) => {{
      ...
      if (href.includes("/project/")) {{
        const encodedName = href.split("/project/")[1].split(/[?#]/)[0];
        window.trackUKInventoryEvent("view_project", {{
          project_name: decodeURIComponent(encodedName),
          page_path: location.pathname,
        }});
      }} else if (label.includes("来源价单")) {{
        window.trackUKInventoryEvent("source_pricelist_click", {{ page_path: location.pathname }});
      }} else if (link.closest("nav")) {{
        window.trackUKInventoryEvent("navigation_click", {{ destination: href }});
      }}
    }});

    document.addEventListener("submit", (event) => {{
      const form = event.target.closest("form.toolbar");
      if (!form) return;
      const activeFilters = Array.from(new FormData(form).values()).filter((value) => String(value).trim()).length;
      window.trackUKInventoryEvent("filter_submit", {{
        page_path: location.pathname,
    ...
    document.addEventListener("change", (event) => {{
      const field = event.target.closest("form.toolbar select, form.toolbar input");
      if (!field) return;
      window.trackUKInventoryEvent("filter_change", {{
        page_path: location.pathname,
        filter_name: field.name || field.dataset.filter || "unknown",
        has_value: field.value ? "yes" : "no",
      }});
    }});

    document.addEventListener("input", (event) => {{
      const field = event.target.closest('form.toolbar input[name="q"], form.toolbar input[data-filter="q"]');
      if (!field) return;
      clearTimeout(searchTimer);
      searchTimer = setTimeout(() => {{
        const searchLength = field.value.trim().length;
        if (searchLength >= 2) {{
          window.trackUKInventoryEvent("site_search", {{
            page_path: location.pathname,
...
def render_units(data: dict, query: dict[str, list[str]] | None = None) -> bytes:
    query = query or {}
    filters = {
    ...
    change_options = "".join(
        f'<option value="{value}"{selected(value, filters["change_type"])}>{label}</option>'
        for value, label in change_labels
    )
